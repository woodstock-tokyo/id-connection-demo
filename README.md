...
We add a **uid** and **token** query parameter in webview url, **uid** is used to pass woodstock user id while **token** is for url validation. **uid** is a **numeric** which should be < 20000 just in case there would be duplication on Alpaca side
